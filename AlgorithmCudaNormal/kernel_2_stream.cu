...
*/
__global__ void loop_2_stream(int* matDst, int *matSrc, int width, int height, int devMatWidth, int devMatHeight, int offsetY)
{
...
void process_2_stream(ALGORITHM_CUDA_NORMAL_PARAM *param, int width, int height)
{
	...
	*/
	...
	for (int i = 0; i < NUM_STREAM; i++) {
		...
		loop_2_stream << < grid, block, 0, *pStream >> > (param->devMatDst, param->devMatSrc, width, height, devMatWidth, devMatHeight, offsetY);
